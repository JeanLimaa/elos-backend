...
export class SupportRequestsController {
  ...
  @ApiBody({ description: 'Dados para criação da solicitação de apoio', type: CreateSupportRequestDto })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({ summary: 'Listar todas as solicitações de apoio (Todas solicitações do usuario. No caso de ADMINS, retorna todas as solicitações pendentes)' })
  @ApiResponse({
    status: 200,
  ...
  @ApiParam({ name: 'id', description: 'ID da solicitação de apoio' })
  @ApiBody({ description: 'Dados para atualização do status da solicitação de apoio', type: UpdateSupportRequestStatusDto })
  @ApiResponse({
    status: 200,
